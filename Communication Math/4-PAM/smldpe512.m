...
for i=1:N
  ...
  if (temp<0.25)               % generate the messages 0
    dsource(i)=0;           		
  elseif (0.25<=temp && temp<0.5)                    % <======== your codes, generate the messages 1
    dsource(i)=1;     		                % <======== your codes
  elseif (0.5<=temp && temp<0.75)                   % <======== your codes, generate the messages 2
...
for i=1:N
  ...
  elseif (-2*d<=r && r < 0)                   % <======== your codes
    decis=1;		     		% Decision is message 1
  elseif (0<=r && r<2*d)                   % <======== your codes
