...
void LPF(Complex *l_X, Complex *r_X, int gain, int M){
    int cutoff = (M * N / 44100)-1;
    for(int k = 0; k < N; k++){
...
int main(int argc, char *argv[]){
    ...
    Stereo *x_afterFFt = calloc(size, sizeof(Stereo));
    Complex *freq_l;
    Complex *freq_r;
    ...
    fwrite(x_afterFFt, 1, header.subChunckSize2, f_output);
    fclose(f_output);
